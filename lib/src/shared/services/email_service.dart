...
class EmailService {
  ...
  Future<void> initialize() async {
    try {
      // Intentar cargar .env solo si no está cargado
  ...
  Future<bool> sendEmail({
    ...
    try {
      // Si no está inicializado, saltarlo
      if (!_isInitialized) {
        print('[Email] No inicializado, saltando envío');
        return false;
      }

      // Verificar que las credenciales están configuradas
      if (_fromEmail == 'noreply@marketmove.app' || _fromEmail.isEmpty) {
        print('[Email] Credenciales no configuradas, saltando envío');
        return false;
      }
